                    ...
                    <div className="poliza-menu" style={{ position: 'relative' }}>
                      ...
                      {menuAbierto === p.id && (
                        <div style={{ position: 'absolute', right: 0, top: '100%', zIndex: 100, background: 'white', borderRadius: 10, border: '1px solid var(--border)', boxShadow: '0 8px 24px rgba(15,30,53,.12)', overflow: 'hidden', minWidth: 140 }}>
                          <button onClick={() => { setEditando(p); setEditForm({ numero: p.numero, ramo: p.ramo, compania: p.compania, corredor: p.corredor, moneda: p.moneda, vencimiento: p.vencimiento, nota: p.nota }); setMenuAbierto(null) }}
...
FILEEOF
git add .
git commit -m 'fix: menu tres puntos funcional + header tabla'
git push
