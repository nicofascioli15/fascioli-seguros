...
/* ── MODALS ── */
.pago-overlay { position: fixed; inset: 0; background: rgba(15,30,53,.5); backdrop-filter: blur(3px); display: flex; align-items: center; justify-content: center; z-index: 280; opacity: 0; pointer-events: none; transition: opacity .18s; }
...
.bottom-nav {
  ...
  height: 64px; background: var(--bg-card); z-index: 220;
...
@media (max-width: 768px) {
  ...
  .main-content {
    padding: 64px 16px calc(72px + 16px) !important;
  ...
  /* ── MODALS ── */
  .pago-overlay { align-items: flex-end !important; padding: 0 0 64px !important; }
  .pago-modal {
    width: 100% !important; max-width: 100% !important;
    border-radius: 20px 20px 0 0 !important;
    max-height: calc(90vh - 64px) !important; overflow-y: auto !important;
...
FILEEOF
git add .
git commit -m 'fix modal no queda tapado por bottom nav en mobile'
git push
