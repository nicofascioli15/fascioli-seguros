        ...
        <div className="pago-overlay open" onClick={e => { if (e.target === e.currentTarget) { setShowPassModal(null); setNewPassword('') } }}>
          <div className="pago-modal" style={{ width: 420 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 20 }}>
              ...
              <button onClick={() => { setShowPassModal(null); setNewPassword('') }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--slate)' }}><X size={18} /></button>
            </div>
            <div style={{ fontSize: 13, color: 'var(--slate)', marginBottom: 16, padding: '10px 14px', background: '#F4F7FB', borderRadius: 8, borderLeft: '3px solid var(--gold)' }}>
              <div style={{ fontWeight: 700, color: 'var(--navy)' }}>{showPassModal.nombre || showPassModal.email}</div>
