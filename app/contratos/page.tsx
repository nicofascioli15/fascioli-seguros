...
import { AlertTriangle, Bell, CheckCircle2, Building2, ArrowUpDown, Accessibility, FileCog, HardHat } from 'lucide-react'
import { createClient } from '@/lib/supabase'
import { CATEGORIAS, esAutoRenovable, calcularAuto, calcularObra, hoyISO } from '@/lib/contratosConfig'
...
export default function ContratosDashboard() {
  ...
  async function fetchAll() {
    setLoading(true)
    const hoy = hoyISO()
    const [{ count: edifCount }, { data: contratosData }] = await Promise.all([
  ...
  const iconPorCategoria: Record<string, any> = { ascensor: ArrowUpDown, rampa: Accessibility, servicio: FileCog, obra: HardHat }
  ...
  const statCards = [
    ...
    { label: 'Por vencer / en ejecución', value: porVencer, sub: '≤95 días o en curso', icon: AlertTriangle, bg: '#FEE2E2', iconColor: '#D94F4F', href: '/contratos/ascensores' },
    { label: 'Seguimiento / garantía', value: seguimiento, sub: '≤180 días o en garantía', icon: Bell, bg: '#FEF3C7', iconColor: '#D97706', href: '/contratos/ascensores' },
    { label: 'Vigentes', value: vigentes, sub: 'Sin novedades', icon: CheckCircle2, bg: '#E6F5EF', iconColor: '#1A7A4E', href: '/contratos/ascensores' },
  ]
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>Ascensores, rampas, servicios y obras — vigencia calculada en vivo</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
        {CATEGORIAS.map(cat => {
          const Icon = iconPorCategoria[cat.value]
          return (
            <a key={cat.value} href={cat.ruta} style={{ textDecoration: 'none' }}>
              <div className="table-card" style={{ padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer' }}>
                ...
                <div>
                  <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-main)' }}>{loading ? '—' : (porCategoria[cat.value] || 0)}</div>
      ...
      {!loading && Object.values(porCategoria).reduce((a, b) => a + b, 0) === 0 && (
        <div style={{ textAlign: 'center', padding: '60px', color: 'var(--text-muted)', background: 'var(--bg-card)', borderRadius: 12, border: '1px solid var(--border-soft)', marginTop: 20 }}>
          ...
          <div style={{ fontSize: 12 }}>Empezá desde Edificios o directamente desde Ascensores</div>
        </div>
      )}
