...
import { CATEGORIAS, Categoria } from '@/lib/contratosConfig'

type Item = { id: string; nombre: string }

const SECCIONES: { categoria: Categoria; titulo: string; abrev: string; placeholder: string }[] = CATEGORIAS.map(c => ({
  categoria: c.value,
  titulo: `Empresas — ${c.label}`,
  abrev: c.value.slice(0, 3).toUpperCase(),
  placeholder: 'Ej: Otis, TKE, Delta...',
}))

function Seccion({ categoria, titulo, abrev, placeholder }: typeof SECCIONES[0]) {
  ...
  async function fetch() {
    setLoading(true)
    const { data } = await supabase.from('contratos_empresas').select('id, nombre').eq('categoria', categoria).order('nombre')
    ...
    <div>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>
          Catálogos propios de Contratos — un listado de empresas por categoría
        </p>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
        {SECCIONES.map(s => <Seccion key={s.categoria} {...s} />)}
      </div>
    </div>
  )
}
