#!/bin/bash
set -e
echo 'Actualizando autofill cuotas...'
...
// Suma N meses a una fecha YYYY-MM-DD manteniendo el día
// Suma N meses y N días extra (uno por cada cuota siguiente)
// Ej: base 15/07, i=1 → 16/08 | i=2 → 17/09 | i=3 → 18/10
function addMonthsAndDays(dateStr: string, months: number): string {
  const [y, m, d] = dateStr.split('-').map(Number)
  // día base + número de cuota (months = índice de cuota)
...
export default function ClienteDetalle({ id, nombre, onBack }: Props) {
  ...
  async function guardarPoliza() {
    ...
    if (!polizaForm.ramo) return
    if (!polizaForm.compania) return
    if (!polizaForm.corredor) return
    if (!polizaForm.vencimiento) return
    const nCuotas = parseInt(polizaForm.cuotas) || 0
    if (nCuotas < 1) { showToast('Error: ingresá al menos 1 cuota'); return }
    if (!polizaForm.fechasCuotas[0]) { showToast('Error: ingresá la fecha de la primera cuota'); return }
    const fechasFaltantes = polizaForm.fechasCuotas.slice(0, nCuotas).filter(f => !f).length
    if (fechasFaltantes > 0) { showToast(`Error: faltan ${fechasFaltantes} fechas de cuotas`); return }
...
echo 'Listo:'
echo '   git add .'
echo '   git commit -m "feat: autofill fechas cuotas dia+mes"'
echo '   git push'
