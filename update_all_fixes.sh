#!/bin/bash
set -e
mkdir -p 'app/(app)/clientes' 'app/(app)/polizas'
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button className="btn-outline" style={{ display: 'flex', alignItems: 'center', gap: 6 }}
            ...
            onClick={e => {
              e.stopPropagation()
              setEditando(detalle)
              setEditForm({ numero: detalle.numero, ramo: detalle.ramo, compania: detalle.compania, corredor: detalle.corredor, moneda: detalle.moneda, vencimiento: detalle.vencimiento, nota: detalle.nota })
                ...
                <tr key={p.id} style={{ cursor: 'pointer' }} onClick={() => abrirDetalle(p)}>
                  ...
                  <td style={{ width: 40 }} onClick={e => e.stopPropagation()}>
                    <div className="poliza-menu" style={{ position: 'relative' }}>
                      ...
                      {menuAbierto === p.id && (
                        <div style={{ position: 'absolute', right: 0, top: '100%', zIndex: 100, background: 'white', borderRadius: 10, border: '1px solid var(--border)', boxShadow: '0 8px 24px rgba(15,30,53,.12)', overflow: 'hidden', minWidth: 140 }}>
                          <button onClick={() => { setEditando(p)
                              setEditForm({ numero: p.numero, ramo: p.ramo, compania: p.compania, corredor: p.corredor, moneda: p.moneda, vencimiento: p.vencimiento, nota: p.nota })
                              setMenuAbierto(null)
                              // Load campos del ramo y valores existentes
                          ...
                          <button onClick={() => { eliminarPoliza(p); setMenuAbierto(null) }}
        ...
        <div className="pago-overlay open" onClick={e => { if (e.target === e.currentTarget) setEditando(null) }}>
          <div className="pago-modal" style={{ width: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 14px' }}>
              ...
              <div className="fgroup">
                <label>Ramo</label>
                <select value={editForm.ramo || ''} onChange={e => setEditForm(p => ({...p, ramo: e.target.value}))}>
                  {catalogos.ramos.map((r:string) => <option key={r}>{r}</option>)}
                </select>
              </div>
              <div className="fgroup">
              ...
              <div className="fgroup">
                <label>Vencimiento</label>
                <DatePicker value={editForm.vencimiento || ''} onChange={v => setEditForm(p => ({...p, vencimiento: v}))} />
              </div>
              <div className="fgroup">
                <label>Moneda</label>
                  ...
                  {editCamposRamo.map(campo => (
                    <div key={campo.id} className="fgroup">
                      ...
                      {campo.tipo === 'select' && campo.opciones ? (
                        ...
                        <DatePicker value={editValores[campo.id] || ''} onChange={v => setEditValores(p => ({...p, [campo.id]: v}))} />
                      ) : (
                        <input type={campo.tipo === 'numero' ? 'number' : 'text'}
                          value={editValores[campo.id] || ''}
                          onChange={e => setEditValores(p => ({...p, [campo.id]: e.target.value}))}
                          placeholder={campo.nombre} />
                      )}
                    </div>
                  ))}
...
git add .
git commit -m 'feat: editar poliza desde clientes + campos dinamicos + formato moneda'
git push
