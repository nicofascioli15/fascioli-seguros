...
import { Plus, Search, X, Loader2, Paperclip, ArrowLeft, FileText, CreditCard, Bell, Upload, Download, Trash2, MoreVertical, Pencil } from 'lucide-react'
...
export default function PolizasPage() {
  ...
  useEffect(() => {
    function handleClick(e: MouseEvent) {
      ...
      if (!target.closest('.poliza-menu')) setMenuAbierto(null)
    }
    ...
    return (
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button className="btn-outline" style={{ display: 'flex', alignItems: 'center', gap: 6 }}
            ...
            onClick={e => {
              e.stopPropagation()
              setEditando(detalle)
              setEditForm({ numero: detalle.numero || '', ramo: detalle.ramo || '', compania: detalle.compania || '', corredor: detalle.corredor || '', moneda: detalle.moneda || '', vencimiento: detalle.vencimiento || '', nota: detalle.nota || '', cuotas: detalle.cuotas || 0 })
              setEditPagosCount(detallePagos.length)
              // Parse existing fechas from cuota_mes
              const fechasExist = (detalle.cuota_mes || '').split(' - ').map((item: string) => {
              ...
              setEditFechasCuotas(fechasExist)
      ...
      {editando && (
        <div className="pago-overlay open" onClick={e => { if (e.target === e.currentTarget) setEditando(null) }}>
          <div className="pago-modal" style={{ width: 500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 14px' }}>
              ...
              <div className="fgroup">
                ...
                <input type="number" min={editPagosCount} max={36}
                  value={editForm.cuotas || ''}
                  onChange={e => {
                    const n = parseInt(e.target.value) || 0
                    if (n < editPagosCount) return
                    setEditForm((p: any) => ({...p, cuotas: n}))
                    // Extend fechas array if adding cuotas
                    if (n > editFechasCuotas.length) {
                      const base = editFechasCuotas[0] || ''
                      const extended = Array.from({ length: n }, (_, i) => editFechasCuotas[i] || (base ? addMonthsAndDays(base, i) : ''))
                      setEditFechasCuotas(extended)
                    } else {
              ...
              <div className="fgroup" style={{ gridColumn: 'span 2' }}><label>Nota (opcional)</label>
                <textarea value={editForm.nota || ''} onChange={e => setEditForm((p: any) => ({...p, nota: e.target.value}))} rows={2}
      ...
      <div className="table-card">
        <table>
          <colgroup>
            ...
            <col style={{ width: 120 }} /><col style={{ width: 130 }} /><col style={{ width: 80 }} /><col style={{ width: 130 }} /><col style={{ width: 48 }} />
          </colgroup>
          <thead>
            <tr>
              ...
              return (
                <tr key={p.id} style={{ cursor: 'pointer' }} onClick={() => abrirDetalle(p)}>
                  ...
                  <td style={{ width: 40 }} onClick={e => e.stopPropagation()}>
                    <div className="poliza-menu" style={{ position: 'relative' }}>
                      <button
                        onMouseDown={e => { e.stopPropagation(); e.preventDefault(); setMenuAbierto(menuAbierto === p.id ? null : p.id) }}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px 6px', borderRadius: 6, color: 'var(--slate)', display: 'flex', alignItems: 'center' }}
                        onMouseEnter={e => ((e.currentTarget as HTMLButtonElement).style.background = '#EEF2F8')}
                        onMouseLeave={e => ((e.currentTarget as HTMLButtonElement).style.background = 'none')}
                      >
                        <MoreVertical size={15} />
                      </button>
                      {menuAbierto === p.id && (
                        <div style={{ position: 'fixed', right: 'auto', zIndex: 200, background: 'white', borderRadius: 10, border: '1px solid var(--border)', boxShadow: '0 8px 24px rgba(15,30,53,.15)', overflow: 'hidden', minWidth: 150 }}
                          ref={(el) => {
                            if (el) {
                              const btn = el.previousElementSibling as HTMLElement
                              const rect = btn?.getBoundingClientRect()
                              if (rect) {
                                el.style.left = `${rect.right - 150}px`
                          ...
                          <button
                            style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--navy)', fontFamily: 'inherit' }}
                            ...
                            onMouseDown={e => {
                              ...
                              setMenuAbierto(null)
                              setEditando(p)
                              setEditForm({ numero: p.numero || '', ramo: p.ramo || '', compania: p.compania || '', corredor: p.corredor || '', moneda: p.moneda || '', vencimiento: p.vencimiento || '', nota: p.nota || '', cuotas: p.cuotas || 0 })
                              // Load pagos count
                              supabase.from('pagos').select('id', { count: 'exact', head: true }).eq('poliza_id', p.id).then(({ count }) => setEditPagosCount(count || 0))
                              // Parse fechas
                              const fEx = (p.cuota_mes || '').split(' - ').map((item: string) => {
                                const parts = item.split('/')
                                if (parts.length < 4) return ''
                                const meses: Record<string,string> = { Ene:'01',Feb:'02',Mar:'03',Abr:'04',May:'05',Jun:'06',Jul:'07',Ago:'08',Sep:'09',Oct:'10',Nov:'11',Dic:'12' }
                                const d = parts[1].padStart(2,'0'), m = meses[parts[2]] || '01', y = `20${parts[3]}`
                              ...
                              setEditFechasCuotas(fEx)
                              supabase.from('ramos').select('id').eq('nombre', p.ramo).single().then(({ data: ramoData }) => {
                                if (!ramoData) { setEditCamposRamo([]); setEditValores({}); return }
                                Promise.all([
                                  supabase.from('campos_ramo').select('*').eq('ramo_id', ramoData.id).order('orden'),
                                  supabase.from('poliza_campos').select('campo_id, valor').eq('poliza_id', p.id),
                                ]).then(([{ data: campos }, { data: valores }]) => {
                                  setEditCamposRamo(campos || [])
                                  const map: Record<string,string> = {}
                                  ;(valores || []).forEach((v: any) => { map[v.campo_id] = v.valor })
                          ...
                          <button onMouseDown={e => { e.stopPropagation(); setMenuAbierto(null); eliminarPoliza(p) }}
                            style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--danger)', fontFamily: 'inherit' }}
...
FILEEOF
git add .
git commit -m 'feat: editar cuotas y fechas en poliza con restriccion de pagadas'
git push
